...
replacements = {
    # meta标签
    'The first permanent international arbitration institution dedicated to resolving digital world disputes. 專業, efficient, and globally enforceable arbitration services.': '首個專注於解決數字世界糾紛的永久性國際仲裁機構。專業、高效、全球可執行的仲裁服務。',
    'arbitration,digital disputes,blockchain,AI disputes,online arbitration,DWAC': '仲裁,數字糾紛,區塊鏈,AI爭議,在線仲裁,DWAC',
    ...
    'Training &amp; events discount': '培訓和活動折扣',
    ...
    'Our Mission': '我們的使命',
    'Our Vision': '我們的願景',
    'Establishing rule of law for the digital world': '構建數字世界的法治',
    'When the boundaries of the physical world are no longer obstacles...': '當物理世界的邊界不再是障礙，當人工智慧代理開始締結合約，當非同質化通證（NFT）成為數字資產，當數據如同呼吸般自由跨越國界……數字世界需要自己的裁判者。DWAC 正為此而生。',
    'Our mission is to': '我們的使命是',
    'Our Vision is': '我們的願景是',
    'Digital Justice, Within Reach': '數字正義，觸手可及',
    ...
    'regional offices established': '設立區域辦事處',
    'Want to learn more?': '想了解更多嗎？',
    'Explore our Charter or get in touch with us.': '探索我們的公約或與我們聯繫。',
    'Read the Charter': '閱讀公約',
    'Contact Us': '聯繫我們',
    
    # Charter页面
    'Digital World Convention': '數字世界公約',
    'Digital disputes resolution principles': '數字爭議解決的基本原則',
    'Preamble': '序言',
    ...
    # 其他
    '© 2026 數字世界仲裁中心 (DWAC). All rights reserved.': '© 2026 數字世界仲裁中心 (DWAC). 保留所有權利。',
...
def main():
    ...
    # 处理zh-tw目录下的所有HTML文件
    zh_tw_files = []
    for root, dirs, files in os.walk(os.path.join(base_dir, 'zh-tw')):
        for file in files:
            if file.endswith('.html'):
                zh_tw_files.append(os.path.join(root, file))
    
    modified_count = 0
    for filepath in zh_tw_files:
        if process_file(filepath):
            modified_count += 1
    
    print(f"\nTotal files modified in zh-tw: {modified_count}")
