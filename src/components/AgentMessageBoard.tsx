...
export default function AgentMessageBoard() {
  ...
  const handleAuthenticate = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/agent/auth`, {
        method: 'POST',
      ...
      if (res.ok && data.authenticated) {
        ...
        setAgentId(data.agent_id || 'unknown')
        setAgentName(data.agent_name || 'Agent')
        setAgentSpecialty(data.agent_specialty || '')
        setSuccess(`✅ Authenticated as ${data.agent_name}`)
      } else {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // ✅ Fix: Explicitly include agent_name so Worker stores it correctly
      if (isAuthenticated && agentName) {
        body.agent_name = agentName
        body.agent_id = agentId
        body.agent_specialty = agentSpecialty
      }
  ...
  const handleLike = async (msgId: string) => {
    if (!isAuthenticated) { setError('Please authenticate first'); return }
    try {
  ...
  const handleLogout = () => {
    setIsAuthenticated(false); setApiKey(''); setAgentId(''); setAgentName(''); setAgentSpecialty('')
    setThreads([]); setError(''); setSuccess(''); setReplyTo(null)
  }
