...
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  ...
  return {
    title: profile ? `${profile.name} - ${roleLabel} | DWAC` : '仲裁员简介 | DWAC',
    description: profile?.shortBio || 'DWAC 仲裁员简介',
  }
}

export default function ProfilePageZhCn({ params }: { params: { slug: string } }) {
  const profile = profiles.find(p => p.slug === params.slug)
  if (!profile) return <div className="min-h-screen flex items-center justify-center text-white">未找到该仲裁员</div>
  ...
  const certs = profile.certifications.map((c) => c)
  ...
  return (
    <div className="flex flex-col">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                {(profile.specializations || profile.specializations || []).slice(0,3).map((s, i) => (
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            <div>
              ...
              <div className="text-xs text-slate-400 mt-1">已处理案件</div>
            </div>
            <div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-3 gap-12">
            <div className="md:col-span-2 space-y-10">
              <div>
                <h2 className="font-serif text-2xl font-bold text-navy-800 mb-4">个人简介</h2>
                <p className="text-gray-600 leading-relaxed text-base">{profile.fullBio}</p>
              </div>

              <div className="bg-slate-50 rounded-xl p-6 border border-slate-200">
                <h3 className="font-serif text-lg font-bold text-navy-800 mb-3">⚖️ 仲裁风格</h3>
              ...
              <div className="bg-gold-50 rounded-xl p-6 border border-gold-200">
                <h3 className="font-serif text-lg font-bold text-navy-800 mb-3">🌟 选择DWAC的原因</h3>
                <p className="text-gray-600 leading-relaxed italic">「{profile.whyDwac || ''}」</p>
              </div>

              <div>
                <h2 className="font-serif text-2xl font-bold text-navy-800 mb-4">执业经历</h2>
            ...
            <div className="space-y-6">
              <div className="bg-navy-900 rounded-xl p-6 text-white">
                <h3 className="font-semibold text-gold-400 mb-4 text-sm uppercase tracking-wide">基本信息</h3>
                <div className="space-y-4 text-sm">
                  ...
                  <div>
                    <div className="text-slate-400 mb-1">语言</div>
                    <div className="font-medium">{profile.languages.join(', ')}</div>
                  </div>
                  {profile.dwacId && (
                    <div>
                      <div className="text-slate-400 mb-1">DWAC编号</div>
                      <div className="font-mono text-gold-400">{profile.dwacId}</div>
                    </div>
                  )}
                  <div>
                    <div className="text-slate-400 mb-1">案件数</div>
              ...
              <div className="bg-white rounded-xl p-6 border border-slate-200">
                <h3 className="font-semibold text-navy-800 mb-4 text-sm uppercase tracking-wide">证书与资质</h3>
              ...
              <div className="bg-white rounded-xl p-6 border border-slate-200">
                <h3 className="font-semibold text-navy-800 mb-4 text-sm uppercase tracking-wide">学历与背景</h3>
              ...
              {isArbitrator && (
                <div className="bg-gradient-to-br from-gold-500/10 to-gold-600/5 rounded-xl p-6 border border-gold-200 text-center">
                  <h3 className="font-serif text-lg font-bold text-navy-800 mb-2">指定该仲裁员</h3>
                  <p className="text-sm text-gray-600 mb-4">希望由 {profile.name.split(' ').pop()} 处理您的案件？</p>
                  <Link
                    href="/zh-cn/contact/"
              ...
              {!isArbitrator && (
                <div className="bg-gradient-to-br from-cyan-500/10 to-cyan-600/5 rounded-xl p-6 border border-cyan-200 text-center">
                  ...
                  <p className="text-sm text-gray-600 mb-4">成为全球社区的一员。</p>
                  <Link
      ...
      <section className="py-6 bg-navy-900 text-gray-400 text-sm">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2">
            <Link href="/zh-cn/" className="hover:text-gold-400 transition-colors">DWAC</Link>
            <span>/</span>
            <Link href="/zh-cn/arbitrators/" className="hover:text-gold-400 transition-colors">仲裁员名册</Link>
            <span>/</span>
